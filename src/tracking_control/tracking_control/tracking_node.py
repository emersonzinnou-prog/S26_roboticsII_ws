...
class TrackingNode(Node):
    ...
    def controller(self, obs_pose, goal_pose):
        ...
        if not obs_pose is None:
            ...
            if np.linalg.norm(dis_obj) - 0.2 < Q:
                U_grad = U_grad + 0.5*n*(1/Q - 1/(np.linalg.norm(dis_obj))-0.2)*1/(np.linalg.norm(dis_obj)-0.2)**2*dis_obj/(np.linalg.norm(dis_obj))
        ...
        cmd_vel.linear.y = max(-0.15,min(0.15, 0.7*U_grad[1]/np.linalg.norm(U_grad[0:2])))
        #cmd_vel.linear.y = 0
