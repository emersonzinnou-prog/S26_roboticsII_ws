...
class TrackingNode(Node):
    ...
    def controller(self, obs_pose, goal_pose):
        ...
        world_obj_pose = self.robot_world_R@self.obs_pose+np.array([self.robot_world_x,self.robot_world_y,self.robot_world_z])
        ...
        #Potential Field
        U_grad = zetta * (pose - world_goal_pose)
        print(dis_goal)
        if not obs_pose is None:
            dis_obj = np.sqrt((pose[0] - world_obj_pose[0])**2 + (pose[1]-world_obj_pose[1])**2)
        ...
        cmd_vel = Twist()
        cmd_vel.linear.x = max(-1.0,min(1.0,Kp*dis_goal[1]))
